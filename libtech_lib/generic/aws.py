...
def get_aws_parquet(filename, bucket_name=None):
    """This function will read the aws parquet file"""
    if bucket_name is None:
        bucket_name = AWS_DATA_BUCKET
    filepath = f"s3://{bucket_name}/{filename}"
    dataframe = wr.pandas.read_parquet(filepath)
    return dataframe
